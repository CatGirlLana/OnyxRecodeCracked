...
class KeyBind(var xPos: Int, var yPos: Int, var xSize: Int, var ySize: Int, @Nullable var text: String?, var textCentered: Boolean, var tooltip: String) : Drawable, HelixUiElement {
    var textColor: Int = 0
    var listening = false
    var bind: Int? = null
    var bindText = "None"
    ...
    override fun render(context: DrawContext, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        if (text != null) {
            if (textCentered) {
                RenderingEngine.Text.draw(context, text!! + bindText, x1 + xSize / 2 - Onyx.MC.textRenderer.getWidth(text!! + bindText) / 2 - 4, y1 + ySize / 2 - 4, textColor)
            } else {
                RenderingEngine.Text.draw(context, text!! + bindText, x1 + 2, y1 + ySize / 2 - 4, textColor)
            }
        }
    }
    ...
    override fun update(xPos: Int, yPos: Int): KeyBind {
        this.xPos = xPos
        this.yPos = yPos
