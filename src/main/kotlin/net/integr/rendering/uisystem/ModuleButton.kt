...
class ModuleButton(var xPos: Int, var yPos: Int, var xSize: Int, var ySize: Int, @Nullable var text: String?, var textCentered: Boolean, var tooltip: String, var outlined: Boolean, var module: Module, var runnable: Runnable) : Drawable, HelixUiElement {
    ...
    override fun render(context: DrawContext, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        if (text != null) {
            if (outlined) {
                if (textCentered) {
                    RenderingEngine.Text.draw(context, text!!, x1 + xSize / 2 - Onyx.MC.textRenderer.getWidth(text) / 2 - 4, y1 + ySize / 2 - 4, colorEnabled)
                } else {
                    RenderingEngine.Text.draw(context, text!!, x1 + 2, y1 + ySize / 2 - 4, colorEnabled)
                }
            } else {
                if (textCentered) {
            ...
            if (module.settings.options.isNotEmpty()) RenderingEngine.Text.draw(context, "" + Formatting.BOLD + "⁝", x1 + xSize-15, y1 + ySize / 2 - 4, textColor)
        }
    }
        ...
        if (button == GLFW.GLFW_MOUSE_BUTTON_RIGHT) {
            ...
            if (mouseX.toInt() in (x1 + 1)..<x2 && mouseY > y1 && mouseY < y2) {
                if (module.settings.options.isNotEmpty()) {
                    runnable.run()
                    Onyx.MC.setScreen(ModuleScreen(module))
                    Onyx.MC.soundManager.play(PositionedSoundInstance.master(SoundEvents.UI_BUTTON_CLICK, 1.0F))
                }
            }
        }
